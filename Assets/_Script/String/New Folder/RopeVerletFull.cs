...
/// <summary>
/// RopeVerletFull
/// - Verlet rope simulation (positions + oldPositions)
/// - Iterative constraints
/// - Force-based bridge: Hooke + damping -> Rigidbody2D
/// - Detect over-stretch; supports cutting at arbitrary node or auto-break at over-stretch
/// - When splitting, preserves per-node velocities; can optionally create mid-anchor with Rigidbody2D
/// </summary>
[RequireComponent(typeof(LineRenderer))]
public class RopeVerletFull : MonoBehaviour
{
    [Header("Anchors")]
    ...
    public bool autoBreakEnabled = true; // auto detect over-stretch
    public bool breakDetectionEnabled = true; // can be toggled (used when cut creates fragments)
    ...
    public bool singleUseBreak = true; // if true original gets destroyed on automatic break
    [Tooltip("If true, mid anchor created when cut will have a Rigidbody2D (so fragments can be physical).")]
    public bool midAnchorHasRigidbody = false;
    public float midAnchorMass = 1f; // mass for mid-anchor rigidbody if created
    ...
    private float lastFixedDt = 0.02f; // track last FixedDeltaTime (used to compute preserved velocities)
    ...
    void FixedUpdate()
    {
        ...
        if (autoBreakEnabled && breakDetectionEnabled)
        {
            ...
            if (seg >= 0)
            {
                // break at segment index seg (between seg and seg+1)
                SplitAtSegment(seg);
                if (singleUseBreak)
                {
                    Destroy(gameObject);
    ...
    void ApplyConstraints()
    {
        ...
        for (int i = 0; i < lastIndex; i++)
        {
            ...
            {
                // split correction evenly for non-anchor segments (including last segment which is partially free)
    ...
    // Force-based Hooke + damping applied to Rigidbody2D (smooth coupling)
    ...
    // detect first over-stretched segment (between i and i+1)
    ...
    public void SplitAtSegment(int segmentIndex)
    {
        ...
        // right positions: segmentIndex+1 .. lastIndex
        ...
        // create mid anchor at midpoint for visuals / anchor
        ...
        if (midAnchorHasRigidbody)
        {
            ...
            // approximate mid velocity as average of adjacent nodes
            Vector2 midVel = 0.5f * (leftVel[leftCount - 1] + rightVel[0]);
            midRb.linearVelocity = midVel;
        }

        // create left fragment
        GameObject leftGO = new GameObject(name + "_fragL");
        CopyLineRenderer(leftGO);
        RopeVerletFull leftRope = leftGO.AddComponent<RopeVerletFull>();
        ...
        RopeVerletFull rightRope = rightGO.AddComponent<RopeVerletFull>();
        CopyConfigTo(rightRope);
        rightRope.startPoint = midT;
        // right fragment inherits original end (rigidbody or transform)
        ...
        // optionally disable break detection for a few frames to avoid immediate re-break
        StartCoroutine(DisableBreakForFrames(leftRope, 3));
        StartCoroutine(DisableBreakForFrames(rightRope, 3));

        // if you want midAnchor to be dynamic and to connect pieces via joint, you can add joints here
    }

    // Cut at a node index (0..lastIndex). nodeIndex must be 1..lastIndex-1
    public void CutAtNode(int nodeIndex)
    {
        ...
        // We want to split between nodeIndex-1 and nodeIndex, but it's simpler to call SplitAtSegment(nodeIndex-1)
        SplitAtSegment(nodeIndex - 1);
        // destroy original if desired
        if (singleUseBreak) Destroy(gameObject);
    }

    // Cut at the node nearest to world position
    ...
    // Initialize rope using given positions and optional velocities (to preserve motion at split)
    public void InitializeFromPositions(Vector2[] posArray, Vector2[] velArray = null)
    {
        ...
        // compute approximate rope length from provided positions
        float len = 0f;
    ...
    // Copy visual LineRenderer settings to new GameObject
    ...
    // Copy runtime config/tuning to the other instance
    void CopyConfigTo(RopeVerletFull other)
    {
    ...
    IEnumerator DisableBreakForFrames(RopeVerletFull rope, int frames)
    {
    ...
    void OnDrawGizmosSelected()
    {
        ...
        for (int i = 0; i < positions.Length; i++)
            Gizmos.DrawSphere((Vector3)positions[i], nodeGizmoSize);
    }
}