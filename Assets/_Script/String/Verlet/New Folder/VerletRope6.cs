...
/// </summary>
[System.Serializable]
public struct RopeNode3
{
    public Vector2 position;
    public Vector2 oldPosition;
    public bool isPinned;

    public RopeNode3(Vector2 pos, bool pinned = false)
    {
        position = pos;
        oldPosition = pos;
        isPinned = pinned;
...
public class VerletRope6 : MonoBehaviour
{
    ...
    public float nodeStepDelay = 0.04f;
    public float nodeMoveSpeed = 6f;
    private int weakNodeIndex = -1;
    ...
    private List<RopeNode3> nodes = new();
    ...
    public void InitializeRope(Vector2 start, Vector2 end, float slackFactor = 1f)
    {
        ...
        for (int i = 0; i < nodeCount; i++)
        {
            ...
            nodes.Add(new RopeNode3(pos));
        }

        maxRopeLength = (nodeCount - 1) * segmentLength * maxStretchFactor;

        // Pin node đầu/cuối nếu có anchor
        if (startPoint) nodes[0] = new RopeNode3(startPoint.position, true);
        if (endPoint) nodes[nodeCount - 1] = new RopeNode3(endPoint.position, true);
    ...
    void SimulateVerlet(float dt)
    {
        ...
        // Giữ anchor
        if (startPoint)
            nodes[0] = new RopeNode3(startPoint.position, true);
        if (endPoint)
            nodes[nodeCount - 1] = new RopeNode3(endPoint.position, true);
    }
    ...
    void ApplyConstraints()
    {
        for (int iter = 0; iter < constraintIterations; iter++)
        {
            ...
            {
                RopeNode3 n1 = nodes[i];
                RopeNode3 n2 = nodes[i + 1];
    ...
    {
        if (isBroken) return; // tránh cắt lần 2

        if (index <= 0 || index >= nodeCount - 1)
            return;

        isBroken = true; // đặt ngay khi bắt đầu cắt

        // 1. Tách danh sách node
        List<RopeNode3> left = new List<RopeNode3>(nodes.GetRange(0, index + 1));
        List<RopeNode3> right = new List<RopeNode3>(nodes.GetRange(index, nodeCount - index));
    ...
    void ApplyRecoil(List<RopeNode3> list, Vector2 recoil)
    {
    ...
    void SpawnRopePiece(List<RopeNode3> nodeList, string nameSuffix)
    {
        GameObject go = new GameObject("RopePiece_" + nameSuffix);
        var rope = go.AddComponent<VerletRope6>();
        ...
        rope.nodes = new List<RopeNode3>(nodeList);
    ...
    public void CheckAutoBreak(Transform anchorDetach = null)
    {
        ...
        weakNodeIndex = FindWeakNodeIndexByActiveLength(weakNodePercent / 100);

        if (weakNodeIndex < 1 || weakNodeIndex >= nodeCount - 2)
        {
            Debug.LogWarning($"[{name}] Overstretched but no valid middle node to cut.");
            return;
        }

        int targetNode = weakNodeIndex + 1;
    ...
    private int GetLastPinnedIndex()
    {
        for (int i = nodes.Count - 1; i >= 0; i--)
        {
            if (nodes[i].isPinned) return i;
        }
        return -1;
    }

    // --- Tính tổng chiều dài phần active (từ lastPinnedIndex -> last node) ---
    public float GetActiveLength()
    {
        if (nodes == null || nodes.Count < 2) return 0f;
        int start = GetLastPinnedIndex();
        if (start < 0) start = 0;
        float len = 0f;
    ...
    // --- Tìm node tại khoảng cách distance tính từ first active node (lastPinnedIndex) ---
    public int FindNodeAtDistance(float distance)
    {
        if (nodes == null || nodes.Count < 2) return -1;
        int first = GetLastPinnedIndex();
        if (first < 0) first = 0;

        float acc = 0f;
        for (int i = first; i < nodes.Count - 1; i++)
        {
            float d = Vector2.Distance(nodes[i].position, nodes[i + 1].position);
            acc += d;
            if (acc >= distance) return i + 1; // trả index node nằm sau đoạn vượt ngưỡng
        }
        return nodes.Count - 1; // nếu vượt hết thì trả node cuối
    }

    int FindWeakNodeIndexByActiveLength(float weakRatio)
    {
        // Bước 1: Xác định node đầu tiên chưa pin
        int firstActive = -1;
        for (int i = 0; i < nodes.Count; i++)
        {
            ...
            {
                firstActive = i;
                Debug.Log("firstActive " + firstActive);
                break;
            }
        }

        // Nếu tất cả đều pin thì không có dây hoạt động
        if (firstActive == -1)
            return nodes.Count - 1;

        // Bước 2: Tính tổng độ dài của phần dây hoạt động
        float totalLength = 0f;
        for (int i = firstActive + 1; i < nodes.Count; i++)
        {
            totalLength += Vector2.Distance(nodes[i].position, nodes[i - 1].position);
        }
        int totalNode = (nodes.Count + firstActive);

        int weakIndex = Mathf.RoundToInt(totalNode * weakNodePercent / 100f) + firstActive;
        // Bước 3: Tìm vị trí node ở 45% chiều dài hoạt động
        float targetDistance = totalLength * weakRatio;
        float accumulated = 0f;

        for (int i = firstActive + 1; i < nodes.Count; i++)
        {
            accumulated += Vector2.Distance(nodes[i].position, nodes[i - 1].position);
